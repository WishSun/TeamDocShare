...
属性名 | 属性含义 | 可见性 | 类型   
------|---------|--------|----- 
m_IP  | 服务器监听监听地址 | private | char[32] 
m_nPort | 服务器监听端口 | private | int  
m_nListenFd | 服务器监听套接字描述符 | private | int 
m_nEpollFd | epoll事件监听集合句柄 | private | int  
m_pBigThrPool | 大任务线程池对象 | private | ThreadPool*  
...
属性名 | 属性含义 | 可见性 | 类型   
------|---------|--------|-----  
m_nPType | 请求类型，包括请求文件、上传文件等等 | public | PType  
m_nFileFd | 已打开的文件描述符，上传文件时，是从客户端连接描述符接收数据然后写入该描述符;请求文件时，是从该文件描述符中读取数据然后发送到客户端连接描述符中 | public | int  
m_nSockFd | 客户端连接套接字描述符  | public | int  
...
m_fileMD5  | 文件的MD5码，用于实现秒传 | public | char[16]  
m_nContentLength | 后边的数据实体的字节数 | public | int64_t  
m_nBytesHaveProcess | 已处理大小字节数 | public | int64_t  
...
属性名 | 属性含义 | 可见性 | 类型   
------|---------|--------|-----  
m_pMyProtocol | 该任务对象应处理的协议包 | private | MyProtocol*  
...
m_nMaxLength | 该线程的任务链表的最大长度 | private | int  
...
m_nCount | 当繁忙任务链表轮转的次数是m_nCount的整数倍时，则切换繁忙任务链表和空闲任务链表的身份，即交换m_nFreeIdx和m_nBusyIdx的值 | private | int  
m_nFreeIdx | 空闲任务链表下标  | private | int  
m_nBusyIdx | 繁忙任务链表下标  | private | int  
...
m_nBigThrNum | 大任务线程个数 | private | int  
m_nSmaThrNum | 小任务线程个数 | private | int  
...
m_nMaxConnNum | 最大支持多少个数据库连接，达到这个数目后，数据库连接数不可再增加，如果有任务需要调用，则必须等待。 | private | int  
